...
export async function requestOrderReturn(id,email,reason='',evidenceUrl=''){await ensureSchema();const r=await getSql()`UPDATE orders SET return_status='REQUESTED',return_requested_at=NOW(),return_reason=${reason},return_evidence_url=${evidenceUrl} WHERE id=${id} AND LOWER(customer->>'email')=LOWER(${email}) AND order_status='DELIVERED' AND delivered_at IS NOT NULL AND delivered_at>=NOW()-INTERVAL '10 days' AND COALESCE(return_status,'')='' RETURNING *`;return mapOrder(r[0])}
export async function updateOrderReturnStatus(id, returnStatus) {
  await ensureSchema();
